...
import { Grid, Menu, Divider, Segment, Header } from "semantic-ui-react";
...
export default class TeamLeadsAndMembers extends PureComponent {
  state = {
  ...
  addShadow() {
    ...
    for (let i = 0; i < imgBg.length; i++) {
      ...
      if (imgBg[i].style.boxShadow === "") {
        ...
        document
          .getElementsByClassName("membercard")[i].addEventListener("click", (e) => {
            if (
              document.getElementsByClassName("membercard")[i].style
                .webkitTransform === "rotateY(0.5turn)"
            ) {
              document.getElementsByClassName("membercard")[i].style.cssText =
              ...
              setTimeout(() => {
                document.getElementsByClassName("membercard")[
                  i
                ].style.cssText =
                  "-webkit-transform: rotateY(0turn);transform: rotateY(0turn);box-shadow:0px 0px 0px 0px #c1930c;border-radius: 5px;";
              }, 3000);
      ...
      <div className="secondAboutPage">
        ...
        <Grid centered doubling stackable>
          <Grid.Row columns={3}>
            ...
            >
              <MemberCard data={faculty_details} />
            </Grid.Column>
            ...
            <Segment inverted raised size="medium">
              <Header as={"h1"} style={{ textAlign: "justiy" }}>
                &quot;Many ideas grow better, <br></br> <pre></pre> when
                transplanted into another mind than the one where<br></br> they
                sprang up. &quot; ~ Oliver Wendell
              </Header>
            </Segment>
            <br></br>
          </Grid.Row>
